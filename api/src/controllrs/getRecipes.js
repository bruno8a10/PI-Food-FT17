       ...
       const apiR = api.data.results
           try{
                for(let i = 0; i< apiR.length;i++){
                 ...
                 response.push({ id:apiR[i].id, name:apiR[i].title, image: apiR[i].image,
                 summary:d.data.summary,spoonacularScore:d.data.spoonacularScore,
                 healthScore: d.data.healthScore, analyzedInstructions: d.data.analyzedInstructions.map((x)=>x.steps.map((c)=>c.step))
                 })   
                }
